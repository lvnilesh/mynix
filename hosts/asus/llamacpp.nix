...
#
# Boot default: carnice27 has wantedBy = ["multi-user.target"] so it starts on boot.
# qwen35 and gemma431 do NOT start on boot. To change the boot default, move
# the wantedBy line from carnice27 to another service and rebuild. Only one can
...
in {
  systemd.services.qwen35 = {
    ...
    conflicts = ["qwen27.service" "gemma431.service" "carnice27.service" "ollama.service"];
    environment = {
  ...
  systemd.services.qwen27 = {
    ...
    wants = ["network.target"];
    requires = ["nvidia-persistenced.service"];
    conflicts = ["qwen35.service" "gemma431.service" "carnice27.service" "ollama.service"];
    environment = {
      LD_LIBRARY_PATH = cudaLibs;
      CUDA_VISIBLE_DEVICES = "0"; # RTX 4090 only
    };
  ...
  systemd.services.gemma431 = {
    ...
    wants = ["network.target"];
    requires = ["nvidia-persistenced.service"];
    conflicts = ["qwen27.service" "qwen35.service" "carnice27.service" "ollama.service"];
    environment = {
      LD_LIBRARY_PATH = cudaLibs;
      CUDA_VISIBLE_DEVICES = "0"; # RTX 4090 only
    };
  ...
  systemd.services.carnice27 = {
    wantedBy = ["multi-user.target"];
    description = "Carnice 27B Model Server (llama.cpp)";
    after = ["network.target" "nvidia-persistenced.service"];
    wants = ["network.target"];
    requires = ["nvidia-persistenced.service"];
    conflicts = ["qwen27.service" "qwen35.service" "gemma431.service" "ollama.service"];
    environment = {
      LD_LIBRARY_PATH = cudaLibs;
    ...
    serviceConfig = {
      ...
      Group = "users";
      WorkingDirectory = inferenceDir;
      ExecStart = "${scriptsDir}/carnice27";
      MemoryMax = "24G";
      MemorySwapMax = "0";
      OOMPolicy = "stop";
      Restart = "on-failure";
